...
public final class World implements Disposable {
     ...
     * 
     * static jclass worldClass = 0; static jmethodID shouldCollideID = 0;
     * static jmethodID beginContactID = 0; static jmethodID endContactID = 0;
     * static jmethodID preSolveID = 0; static jmethodID postSolveID = 0; static
     * jmethodID reportFixtureID = 0; static jmethodID reportRayFixtureID = 0;
     * 
     * class CustomRayCastCallback: public b2RayCastCallback { private: JNIEnv*
     * env; jobject obj;
     * 
     * public: CustomRayCastCallback( JNIEnv *env, jobject obj ) { this->env =
     * env; this->obj = obj; }
     * 
     * virtual float32 ReportFixture( b2Fixture* fixture, const b2Vec2& point,
     * const b2Vec2& normal, float32 fraction) { return
     * env->CallFloatMethod(obj, reportRayFixtureID, (jlong)fixture,
     * (jfloat)point.x, (jfloat)point.y, (jfloat)normal.x, (jfloat)normal.y,
     * (jfloat)fraction ); } };
     * 
     * class CustomContactFilter: public b2ContactFilter { private: JNIEnv* env;
     * jobject obj;
     * 
     * public: CustomContactFilter( JNIEnv* env, jobject obj ) { this->env =
     * env; this->obj = obj; }
     * 
     * virtual bool ShouldCollide(b2Fixture* fixtureA, b2Fixture* fixtureB) {
     * if( shouldCollideID != 0 ) return env->CallBooleanMethod( obj,
     * shouldCollideID, (jlong)fixtureA, (jlong)fixtureB ); else return true; }
     * };
     * 
     * class CustomContactListener: public b2ContactListener { private: JNIEnv*
     * env; jobject obj;
     * 
     * public: CustomContactListener( JNIEnv* env, jobject obj ) { this->env =
     * env; this->obj = obj; }
     * 
     * /// Called when two fixtures begin to touch. virtual void
     * BeginContact(b2Contact* contact) { if( beginContactID != 0 )
     * env->CallVoidMethod(obj, beginContactID, (jlong)contact ); }
     * 
     * /// Called when two fixtures cease to touch. virtual void
     * EndContact(b2Contact* contact) { if( endContactID != 0 )
     * env->CallVoidMethod(obj, endContactID, (jlong)contact); }
     * 
     * /// This is called after a contact is updated. virtual void
     * PreSolve(b2Contact* contact, const b2Manifold* oldManifold) { if(
     * preSolveID != 0 ) env->CallVoidMethod(obj, preSolveID, (jlong)contact,
     * (jlong)oldManifold); }
     * 
     * /// This lets you inspect a contact after the solver is finished. virtual
     * void PostSolve(b2Contact* contact, const b2ContactImpulse* impulse) { if(
     * postSolveID != 0 ) env->CallVoidMethod(obj, postSolveID, (jlong)contact,
     * (jlong)impulse); } };
     * 
     * class CustomQueryCallback: public b2QueryCallback { private: JNIEnv* env;
     * jobject obj;
     * 
     * public: CustomQueryCallback( JNIEnv* env, jobject obj ) { this->env =
     * env; this->obj = obj; }
     * 
     * virtual bool ReportFixture( b2Fixture* fixture ) { return
     * env->CallBooleanMethod(obj, reportFixtureID, (jlong)fixture ); } };
     * 
     * inline b2BodyType getBodyType( int type ) { switch( type ) { case 0:
    ...
    public Body createBody(BodyDef def) {
        long bodyAddr = jniCreateBody(addr, def.type.getValue(), def.position.x, def.position.y, def.angle, def.linearVelocity.x, def.linearVelocity.y,
                def.angularVelocity, def.linearDamping, def.angularDamping, def.allowSleep, def.awake, def.fixedRotation, def.bullet, def.active,
                def.gravityScale);
    ...
    private native long jniCreateBody(long addr, int type, float positionX, float positionY, float angle, float linearVelocityX, float linearVelocityY,
            float angularVelocity, float linearDamping, float angularDamping, boolean allowSleep, boolean awake, boolean fixedRotation, boolean bullet,
            boolean active, float inertiaScale); /*
                                                  ...
                                                  * angularDamping;
                                                  * bodyDef.allowSleep =
                                                  * allowSleep; bodyDef.awake =
                                                  * awake; bodyDef.fixedRotation
                                                  * = fixedRotation;
    ...
    public void destroyBody(Body body) {
        ...
        while (!fixtureList.isEmpty()) {
            this.fixtures.remove(fixtureList.remove(0).addr)
                         .setUserData(null);
        }
        List<JointEdge> jointList = body.getJointList();
        while (!jointList.isEmpty())
            destroyJoint(body.getJointList()
                             .get(0).joint);
    ...
    private long createProperJoint(JointDef def) {
        if (def.type == JointType.DistanceJoint) {
            ...
            return jniCreateDistanceJoint(addr, d.bodyA.addr, d.bodyB.addr, d.collideConnected, d.localAnchorA.x, d.localAnchorA.y, d.localAnchorB.x,
                    d.localAnchorB.y, d.length, d.frequencyHz, d.dampingRatio);
        }
        if (def.type == JointType.FrictionJoint) {
            FrictionJointDef d = (FrictionJointDef) def;
            return jniCreateFrictionJoint(addr, d.bodyA.addr, d.bodyB.addr, d.collideConnected, d.localAnchorA.x, d.localAnchorA.y, d.localAnchorB.x,
                    d.localAnchorB.y, d.maxForce, d.maxTorque);
        }
        if (def.type == JointType.GearJoint) {
        ...
        if (def.type == JointType.PrismaticJoint) {
            ...
            return jniCreatePrismaticJoint(addr, d.bodyA.addr, d.bodyB.addr, d.collideConnected, d.localAnchorA.x, d.localAnchorA.y, d.localAnchorB.x,
                    d.localAnchorB.y, d.localAxisA.x, d.localAxisA.y, d.referenceAngle, d.enableLimit, d.lowerTranslation, d.upperTranslation, d.enableMotor,
                    d.maxMotorForce, d.motorSpeed);
        }
        if (def.type == JointType.PulleyJoint) {
            PulleyJointDef d = (PulleyJointDef) def;
            return jniCreatePulleyJoint(addr, d.bodyA.addr, d.bodyB.addr, d.collideConnected, d.groundAnchorA.x, d.groundAnchorA.y, d.groundAnchorB.x,
                    d.groundAnchorB.y, d.localAnchorA.x, d.localAnchorA.y, d.localAnchorB.x, d.localAnchorB.y, d.lengthA, d.lengthB, d.ratio);

        }
        if (def.type == JointType.RevoluteJoint) {
            RevoluteJointDef d = (RevoluteJointDef) def;
            return jniCreateRevoluteJoint(addr, d.bodyA.addr, d.bodyB.addr, d.collideConnected, d.localAnchorA.x, d.localAnchorA.y, d.localAnchorB.x,
                    d.localAnchorB.y, d.referenceAngle, d.enableLimit, d.lowerAngle, d.upperAngle, d.enableMotor, d.motorSpeed, d.maxMotorTorque);
        }
        if (def.type == JointType.WeldJoint) {
            WeldJointDef d = (WeldJointDef) def;
            return jniCreateWeldJoint(addr, d.bodyA.addr, d.bodyB.addr, d.collideConnected, d.localAnchorA.x, d.localAnchorA.y, d.localAnchorB.x,
                    d.localAnchorB.y, d.referenceAngle);
        }
        if (def.type == JointType.RopeJoint) {
            RopeJointDef d = (RopeJointDef) def;
            return jniCreateRopeJoint(addr, d.bodyA.addr, d.bodyB.addr, d.collideConnected, d.localAnchorA.x, d.localAnchorA.y, d.localAnchorB.x,
                    d.localAnchorB.y, d.maxLength);
        }
        if (def.type == JointType.WheelJoint) {
            WheelJointDef d = (WheelJointDef) def;
            return jniCreateWheelJoint(addr, d.bodyA.addr, d.bodyB.addr, d.collideConnected, d.localAnchorA.x, d.localAnchorA.y, d.localAnchorB.x,
                    d.localAnchorB.y, d.localAxisA.x, d.localAxisA.y, d.enableMotor, d.maxMotorTorque, d.motorSpeed, d.frequencyHz, d.dampingRatio);
        }
    ...
    private native long jniCreateWheelJoint(long addr, long bodyA, long bodyB, boolean collideConnected, float localAnchorAX, float localAnchorAY,
            float localAnchorBX, float localAnchorBY, float localAxisAX, float localAxisAY, boolean enableMotor, float maxMotorTorque, float motorSpeed,
            float frequencyHz, float dampingRatio); /*
                                                     * b2World* world =
                                                     * (b2World*)addr;
                                                     * b2WheelJointDef def;
                                                     * def.bodyA =
                                                     * (b2Body*)bodyA; def.bodyB
                                                     * = (b2Body*)bodyB;
                                                     * def.collideConnected =
                                                     * collideConnected;
                                                     * def.localAnchorA =
                                                     * b2Vec2(localAnchorAX,
                                                     * localAnchorAY);
                                                     * def.localAnchorB =
                                                     * b2Vec2(localAnchorBX,
                                                     * localAnchorBY);
                                                     * def.localAxisA =
                                                     * b2Vec2(localAxisAX,
                                                     * localAxisAY);
                                                     ...
                                                     * 
                                                     * return
                                                     * (jlong)world->CreateJoint
                                                     * (&def);
                                                     */

    private native long jniCreateRopeJoint(long addr, long bodyA, long bodyB, boolean collideConnected, float localAnchorAX, float localAnchorAY,
            float localAnchorBX, float localAnchorBY, float maxLength); /*
                                                                         * b2World*
                                                                         * world
                                                                         * =
                                                                         * (b2World
    ...
    private native long jniCreateDistanceJoint(long addr, long bodyA, long bodyB, boolean collideConnected, float localAnchorAX, float localAnchorAY,
            float localAnchorBX, float localAnchorBY, float length, float frequencyHz, float dampingRatio); /*
                                                                                                             * b2World
                                                                                                             * *
                                                                                                             * world
                                                                                                             * =
    ...
    private native long jniCreateFrictionJoint(long addr, long bodyA, long bodyB, boolean collideConnected, float localAnchorAX, float localAnchorAY,
            float localAnchorBX, float localAnchorBY, float maxForce, float maxTorque); /*
                                                                                         * b2World
                                                                                         * *
                                                                                         * world
                                                                                         * =
    ...
    private native long jniCreateGearJoint(long addr, long bodyA, long bodyB, boolean collideConnected, long joint1, long joint2, float ratio); /*
                                                                                                                                                 * b2World
                                                                                                                                                 * *
                                                                                                                                                 * world
                                                                                                                                                 * =
    ...
    private native long jniCreateMouseJoint(long addr, long bodyA, long bodyB, boolean collideConnected, float targetX, float targetY, float maxForce,
            float frequencyHz, float dampingRatio); /*
                                                     * b2World* world =
                                                     * (b2World*)addr;
                                                     * b2MouseJointDef def;
    ...
    private native long jniCreatePrismaticJoint(long addr, long bodyA, long bodyB, boolean collideConnected, float localAnchorAX, float localAnchorAY,
            float localAnchorBX, float localAnchorBY, float localAxisAX, float localAxisAY, float referenceAngle, boolean enableLimit, float lowerTranslation,
    ...
    private native long jniCreatePulleyJoint(long addr, long bodyA, long bodyB, boolean collideConnected, float groundAnchorAX, float groundAnchorAY,
            float groundAnchorBX, float groundAnchorBY, float localAnchorAX, float localAnchorAY, float localAnchorBX, float localAnchorBY, float lengthA,
            float lengthB, float ratio); /*
                                          * b2World* world = (b2World*)addr;
                                          * b2PulleyJointDef def; def.bodyA =
                                          * (b2Body*)bodyA; def.bodyB =
                                          * (b2Body*)bodyB; def.collideConnected
                                          * = collideConnected;
                                          * def.groundAnchorA = b2Vec2(
                                          * groundAnchorAX, groundAnchorAY );
                                          * def.groundAnchorB = b2Vec2(
                                          * groundAnchorBX, groundAnchorBY );
                                          * def.localAnchorA =
                                          * b2Vec2(localAnchorAX,
                                          * localAnchorAY); def.localAnchorB =
                                          * b2Vec2(localAnchorBX,
                                          * localAnchorBY); def.lengthA =
                                          * lengthA; def.lengthB = lengthB;
                                          * def.ratio = ratio;
                                          * 
                                          * return
                                          * (jlong)world->CreateJoint(&def);
                                          */

    private native long jniCreateRevoluteJoint(long addr, long bodyA, long bodyB, boolean collideConnected, float localAnchorAX, float localAnchorAY,
            float localAnchorBX, float localAnchorBY, float referenceAngle, boolean enableLimit, float lowerAngle, float upperAngle, boolean enableMotor,
            float motorSpeed, float maxMotorTorque); /*
                                                      ...
                                                      * motorSpeed;
                                                      * def.maxMotorTorque =
                                                      * maxMotorTorque; return
                                                      * (jlong
                                                      * )world->CreateJoint
                                                      * (&def);
                                                      */

    private native long jniCreateWeldJoint(long addr, long bodyA, long bodyB, boolean collideConnected, float localAnchorAX, float localAnchorAY,
     ...
     * to support sub-stepping. Sub-stepping is often used to maintain a fixed
     * sized time step under a variable frame-rate. When you perform
     * sub-stepping you will disable auto clearing of forces and instead call
     * ClearForces after all sub-steps are complete in one pass of your game
     * loop. {@link #setAutoClearForces(boolean)}
     */
    ...
    /**
     * Returns the list of {@link Contact} instances produced by the last call
     * to {@link #step(float, int, int)}. Note that the returned list will have
     * O(1) access times when using indexing. contacts are created and destroyed
     * in the middle of a time step. Use {@link ContactListener} to avoid
     * missing contacts
     * 
    ...
    private boolean contactFilter(long fixtureA, long fixtureB) {
        if (contactFilter != null)
            ...
            Filter filterA = fixtures.get(fixtureA)
                                     .getFilterData();
            Filter filterB = fixtures.get(fixtureB)
                                     .getFilterData();
